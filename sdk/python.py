...
class SimeisSDK:
    ...
    def setup_player(self, username, force_register=False):
        ...
        username = "".join(
            [c for c in username if c in allowed]
        ).lower()
        ...
        if player["money"] <= 0.0:
            print(
                "!!! Player already lost, please restart the server "
                "to reset the game"
            )
            sys.exit(0)
    ...
    def scan_planets(self, sta):
        ...
        return sorted(
            planets,
            key=lambda pla: get_dist(
                station["position"], pla["position"]
            ),
        )
